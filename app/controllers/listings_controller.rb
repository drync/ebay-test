class ListingsController < ApplicationController
  ...
  def create
    ...
    result = try! {
      ebay.add_fixed_price_item(item: build_item(@listing))
    }.
    map { |response|
    ...
    map_err { |e|
      if e.errors.grep(/SKU must be unique/i).any?
        try! { ebay.revise_fixed_price_item(item: build_item(@listing)) }
      else
        Failure(e)
      end
    }
    ...
    result.match do
      Success(s) { redirect_to listings_path }
      Failure(f) { |e|
  ...
  def update
    ...
    begin
      ebay.revise_fixed_price_item(:item => build_item(@listing))
